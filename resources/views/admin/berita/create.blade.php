...
<div class="space-y-6 max-w-5xl mx-auto">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-black text-slate-800 tracking-tight">Tambah Berita</h1>
            <p class="text-sm text-slate-500 mt-1 font-medium">Buat konten publikasi baru</p>
        </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        <form action="{{ route('admin.berita.store') }}" method="POST" enctype="multipart/form-data" class="p-6 sm:p-8">
            @csrf
            ...
            <div class="space-y-6">
                ...
                <!-- Thumbnail -->
                <div>
                    <label for="thumbnail" class="block text-sm font-bold text-slate-700 mb-2">Thumbnail (Opsional)</label>
                ...
                <div class="flex items-center bg-slate-50 p-4 rounded-xl border border-slate-200">
                    ...
                    <label for="is_published" class="ml-3 text-sm font-bold text-slate-700 cursor-pointer">Langsung Publikasikan</label>
                </div>
            </div>
        ...
        $('#konten').summernote({
            ...
            callbacks: {
                onImageUpload: function(files) {
                    // Logic untuk upload image lokal di Summernote jika perlu
                }
            }
        });
    });
