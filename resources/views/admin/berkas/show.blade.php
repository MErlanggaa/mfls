    ...
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <div class="flex items-center gap-3 self-start sm:self-auto">
            <form action="{{ route('admin.pendaftar.destroy', $user->id) }}" method="POST"
                ...
                @method('DELETE')
                <button type="submit"
                    class="inline-flex items-center gap-2 px-5 py-2.5 bg-red-50 border border-red-200 text-red-600 rounded-2xl text-xs font-black shadow-sm hover:bg-red-500 hover:text-white transition-all uppercase tracking-widest">
                    <span class="iconify" data-icon="solar:trash-bin-trash-bold"></span> Hapus Peserta
                </button>
            </form>

            <a href="{{ route('admin.berkas.index') }}"
                class="inline-flex items-center gap-2 px-5 py-2.5 bg-white border border-slate-200 text-slate-700 rounded-2xl text-xs font-black shadow-sm hover:bg-slate-50 transition-all uppercase tracking-widest">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @php
                ...
                if (str_contains($user->peserta->pilihan_prodi ?? '', 'DKV')) {
                    $berkasList['Portofolio Karya'] = $user->peserta->berkas->portfolio ?? null;
                }
            @endphp
            @foreach($berkasList as $name => $path)
                <div
                    class="p-6 rounded-3xl border {{ $path ? 'bg-blue-50/30 border-blue-100' : 'bg-slate-50 border-slate-200 opacity-60' }} transition-all">
                    <div class="flex items-center gap-4 mb-4">
                        <div
                            class="w-12 h-12 {{ $path ? 'bg-blue-600 text-white' : 'bg-slate-300 text-white' }} rounded-2xl flex items-center justify-center shadow-lg text-lg">
                            @if($path) ✓ @else ! @endif
                        </div>
                    </div>
                    <div class="text-lg font-black text-slate-800 mb-1">{{ $name }}</div>
                    <div class="text-xs font-bold {{ $path ? 'text-blue-500' : 'text-slate-400' }} uppercase mb-4">
                        {{ $path ? 'FILE TERSEDIA' : 'BELUM UPLOAD' }}</div>
                    ...
                    @if($path)
                        ...
                        @endphp
                            <a href="{{ $fullUrl }}" target="_blank" class="inline-block px-6 py-2 bg-white border border-blue-200 text-blue-600 rounded-xl text-xs font-black shadow-sm hover:bg-blue-50 transition-all">
                                {{ $isExternal ? 'BUKA LINK ↗' : 'LIHAT DOKUMEN ↗' }}
                                </a>
                    @endif
                    </div>
            @endforeach
