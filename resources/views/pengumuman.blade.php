...
<body class="relative overflow-x-hidden pt-8 md:pt-12">
    ...
    <img src="https://upload.wikimedia.org/wikipedia/commons/a/ad/Indonesia_blank_map.svg" class="watermark-map" alt="Watermark">
    ...
    <main class="flex-grow flex flex-col items-center justify-center p-4 md:p-8 relative z-10">
        ...
        <div class="w-full max-w-xl bg-white rounded-3xl shadow-2xl p-8 border border-slate-100">
            ...
            <form action="{{ route('pengumuman') }}" method="GET" class="space-y-4">
                <div>
                    <label class="text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1">Nomor Induk Siswa Nasional (NISN)</label>
                    <input type="text" name="nisn" maxlength="10" placeholder="Contoh: 0049999XXX" 
                        class="w-full px-6 py-4 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:border-snbp-blue focus:ring-0 transition-all font-bold text-lg text-slate-700 outline-none" required>
                </div>
                <button type="submit" class="w-full bg-snbp-blue hover:bg-blue-900 text-white py-5 rounded-2xl font-black text-lg transition-all shadow-lg flex items-center justify-center gap-3">
        ...
        @else
            ...
            <div class="w-full max-w-lg overflow-hidden rounded-3xl shadow-[0_35px_60px_-15px_rgba(0,0,0,0.3)] transform transition-all duration-500 hover:scale-[1.01]">
                <!-- Card Header -->
                ...
                <div class="bg-snbp-black p-8 text-white relative">
                    <div class="flex flex-col md:flex-row gap-6 items-center md:items-start mb-8">
                        <div class="text-center md:text-left">
                            <p class="text-white/50 text-[10px] font-bold tracking-widest uppercase">NISN {{ $peserta->nisn }} - NOREG {{ $peserta->id }}</p>
                    ...
                    <div class="mt-8 pt-6 border-t border-white/10">
                        <div class="bg-white/5 border border-white/10 rounded-2xl p-5 mb-6">
                            <p class="text-xs text-white/80 leading-relaxed italic text-center">
                                "Selamat atas pencapaian Anda. Silakan unduh surat keterangan kelulusan resmi di bawah ini sebagai bukti verifikasi administrasi."
                            </p>
                        </div>
                        <button onclick="generatePDF()" id="btnDownloadPDF" class="w-full bg-white text-snbp-black hover:bg-slate-200 py-4 rounded-2xl font-black text-sm transition-all flex items-center justify-center gap-3">
                    ...
                    <div class="mt-8 pt-6 border-t border-white/10 text-center">
                        <p class="text-xs text-white/50 leading-relaxed">
                            Jangan patah semangat! Masih ada kesempatan lain atau jalur beasiswa mandiri. Teruslah berusaha untuk masa depan yang gemilang.
                        </p>
                        <a href="{{ route('pengumuman') }}" class="inline-flex items-center gap-2 text-white/70 hover:text-white text-xs font-bold mt-6 underline decoration-white/20 underline-offset-4">
                ...
                <div class="bg-slate-100 p-4 text-center">
                    <button onclick="shareResult('{{ $peserta->nama }}', '{{ $isLulus ? 'LULUS' : 'TIDAK LULUS' }}')" class="flex items-center justify-center gap-2 mx-auto text-slate-500 hover:text-snbp-blue text-[10px] font-black uppercase tracking-widest transition-colors">
                        <span class="iconify" data-icon="solar:share-bold"></span> Bagikan Hasil Seleksi
                    </button>
                </div>
            </div>

            <p class="mt-8 text-slate-400 text-[10px] font-bold uppercase tracking-[0.2em]">© 2026 MNC UNIVERSITY ADMISSION</p>
        @endif
    </main>
